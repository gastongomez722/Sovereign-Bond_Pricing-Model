# -*- coding: utf-8 -*-
"""Nelson Sviegel Svenssonvs vs ridge polynomial regression

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KJ-asg4yG8veaRgZK8PFIBRJNL0PyQjb

Code Developed by Jaun Aramis Delich and Gaston Gomez

Assumptions for polycurve fitting

- Z scaling and centering of yields

- regression against log duration for ridge regression stabilization


Need to fix:

- randomized dropout right now can drop out first and last durations, even though polynomials are only good for interpolation

- LOOCV without leaving out first and last datapoints

- instability with US bond yields, since duplicate durations cannot be interpolated at the threshold (polynomials cannot take on two y values at the same x)

- API requests for STRIP rates and durations

- loop to find ideal polynomial degree, and plot Train VS test Set MSE as a function of degree

- realistic future rate paths, arbitrage free pricing suggest non-negative future rates, current logic does not enforce positive future rates

- scipy tends to spit errors when using very high degree polynomials, either truncation errors which result in non-optimal solutions, or simply switching to ordinary least squares, errors tend to occur past interpolation threshold

Bonus points:

- generalized schema for polyfitting, with Alpha (ridge regression penalization) function of Number of datapoints? polynomial degree? most likely both. for low degree polynomials (below interpolation threshold) high alpha causes the polynomial to only fit end points -> ideal alpha is a function of degree

- comparison of performance to Nelson Siegel Svensson model, using LOOCV

- Using LOOCV to quantify confidence? similar to Gaussian Process Regression

Surprising Findings:
- interpolation and extrapolation of forward rates of regularized polynomials (using alpha = 0.4degree**2) is robust to degree choice and data up until slightly past the interpolation threshold, this suggests that polynomials for interpolation are much better than NSS, since they fit spot rates better and forward rates the same (even for extrapolation), all while having less assumptions than NSS (which explicitely enforces non-negative forward rates)

Results:

- observing double descent as a function of model parameters in ridge regression for yield curve estimation

- ideally find a yield curve estimation method which achieves better or equal performance as traditional methods, such as nelson siegel svensson, using less assumptions (occam's razor)

- generalization for schema of ideal alpha as a function of polynomial degree and number of obs (current schema is 0.4degree*2)
"""
